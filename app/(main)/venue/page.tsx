...
const Venue = () => {
  ...
  const handleCreateVenue = async () => {
    ...
    try {
      ...
      const buildingId=buildings.find((buildin)=>buildin.name===building) as Building
      const props = {
        name:newVenue,
      ...
      const response: any = addNewVenue(props, token as string).then(
        (res: any) => {
          if (res.status == 200) {
            ...
            setAddVenueAlert(true);
            setNewVenue("");
            setResetVenues((val) => !val);
            setTimeout(() => {
              setAddVenueAlert(false);
            }, 2000);
          }
        }
      );
    } catch (error: any) {
      setIsNewVenueError(true);
      setNewVenueError(error.response.data.response_message);
      setTimeout(() => {
        setIsNewVenueError(false);
        setNewVenueError("");
      }, 3000);
    } finally {
      setAddVenueLoading(false);
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Enter the name of the Venue you want to add!
              </DialogDescription>
            </DialogHeader>
              ...
              venues.map((Venue, index) => (
                ...
                >
                  <VenueCard
                    ...
                    seating_capacity={Venue.seating_capacity as Number}
                    is_accessible={Venue.is_accessible as Boolean}
                    has_whiteboard={Venue.has_whiteboard as Boolean}
                    has_projectors={Venue.has_projectors as Boolean}
                    has_speakers={Venue.has_speakers as Boolean}
                    has_air_conditioner={Venue.has_air_conditioner as Boolean}
                    floor_number={Venue.floor_number as Number}
                    building_id={Venue.building_id as string}
                    authority_id={Venue.authority_id as string}
