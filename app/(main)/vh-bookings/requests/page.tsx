...
import { getVHBookingRequestsByUser } from "@/utils/utils";
import VHBookingRequestCard from "@/components/VHBookingRequestCard/page";
...
const BookingRequests = () => {
    const [bookingRequests, setBookingRequests] =
        useState<Array<BookingRequestProps>>();
    useEffect(() => {
        const getBookingRequests = async () => {
            const token = localStorage.getItem("token");
            const user = localStorage.getItem("user");
            const response = await getVHBookingRequestsByUser(
                user as string,
                token as string
            ).then((res: any) => {
                const resp = res;
                        ...
                        <ScrollArea className="h-[550px]">
                            {pendingRequests && pendingRequests.length > 0 ? (
                                pendingRequests.map((request) => (
                                    <div
                                        key={request.id}
                                        className="mt-4 px-20 w-full"
                                    >
                                        <VHBookingRequestCard
                                ...
                                <p className="w-full mt-12 text-center font-bold text-2xl">
                                    No pending requests
                                </p>
                            )}
                        </ScrollArea>
