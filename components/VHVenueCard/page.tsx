...
const VHVenueCard = ({
    ...
    const handleUpdateVHVenue = async () => {
            ...
            const build_id: string =
                buildings.length > 0
                    ? (buildings.find((ele) => ele.name == building)
                          ?.id as string)
                    : "";
            const response: any = UpdateExistingVHVenue(
                id,
                newVenue,
                authority,
                build_id,
                floor,
                accomodationType.toUpperCase(),
                token as string
            ).then((res: any) => {
                const resp = res;
                if (resp.status == 200) {
    ...
    const handleDeleteVHVenue = async () => {
        setDeleteDialogueOpen(false);
        setDeleteVenueLoading(true);
        try {
            const token = localStorage.getItem("token");
            const response: any = deleteVHVenue(id, token as string).then(
                (res: any) => {
                    const resp = res;
                    if (resp.status == 200) {
                        console.log(resp);
